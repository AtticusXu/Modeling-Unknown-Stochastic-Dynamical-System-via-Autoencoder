...
model_dir = model_dir+data_type+ f"_z{l_d}_pdf1d{b_p1}_pdfnd{b_pn}_moment{b_m}_cov{b_c}_mmd{b_d}/"
...
for i in range(2):
    ...
    kde_layer_1d, pdf_normal_c_1d, kde_layer_nd, pdf_normal_c_nd = DEAE_lib.kde_preparation(
                                            ...
                                            kde_range, kde_num, n_sample[i], batch_size)
    
    moment_layer = MomentLayer1D(max_moment, moment_weights)
    cov_layer = CovZLayer()
    mmd_layer = FastMMDLayer(mmd_sigma,mmd_basis)
    
    cvae = ConditionalVAE(encoder, decoder, kde_layer_1d, kde_layer_nd, moment_layer,
                          cov_layer, mmd_layer, pdf_normal_c_1d, pdf_normal_c_nd,
                          moment_normal_c, rnn = RNN, 
                          beta_kde_1d = beta_kde_1d[i], beta_kde_nd = beta_kde_nd[i],
                          beta_moment = beta_moment[i], beta_cov = beta_cov[i],
                          beta_mmd = beta_mmd[i])
    ...
    save_dir = eqn_name+'/'+model_dir+'RNN{}/sample{}/'.format(RNN,n_sample[i])+ensemble_testID
    ...
    hist = cvae_2step[i].fit(x=train_data[i], batch_size=batch_size, epochs=epochs[i], 
                        callbacks = callback_2step[i], verbose=verbose, validation_split=0.1)
    
    cvae_2step[i].save_weights(save_dir_2step[i]+'/cvae_{}epochs.h5'.format(epochs[i]))
    
    with open(save_dir_2step[i]+'loss history.json', 'w') as f:
        json.dump(hist.history, f)
    np.savetxt(save_dir_2step[i]+'/train_loss_{}epochs.csv'.format(epochs[i]),hist.history['loss'])
    np.savetxt(save_dir_2step[i]+'/val_loss_{}epochs.csv'.format(epochs[i]),hist.history['val_loss'])
