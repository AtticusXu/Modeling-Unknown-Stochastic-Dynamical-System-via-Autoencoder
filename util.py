...
class ModelCheckpoint(keras.callbacks.Callback):
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        if self.batches_since_last_save >= self.period:
            self.batches_since_last_save = 0
            if self.rnn:
                current = self.model.evaluate(
                    self.inputs, self.outputs, verbose=0)[-1]
            else:
                current = self.model.evaluate(
                    self.inputs, self.outputs, verbose=0)
            self.history.append(self.model.evaluate(
                self.inputs, self.outputs, verbose=0))
...
class BestModelCheckpoint_v2(tf.keras.callbacks.Callback):
    ...
    def __init__(self, train_inputs, train_outputs, model_dir, epochs_trained,
        ...
        # before this training begin, from last training
    ...
    def on_train_begin(self, logs=None):
        if self.epochs_trained == 0:
            ...
            model_loss = self.model.evaluate(
                self.train_inputs, self.train_outputs, verbose=0)
            self.best_sub_models = []
            if self.rnn:
                self.best_loss = model_loss[0]
            else:
                self.best_loss = model_loss
            self.loss_hist = np.array(model_loss).reshape(
                [1, -1])  # [epoch, loss]
        else:
            self.best_model = tf.keras.models.load_model(
                self.model_dir + name_best_model_epoch(self.epochs_trained))
            self.best_loss = self.best_model.evaluate(
                self.train_inputs, self.train_outputs, verbose=0)
            self.best_sub_models = []
            if self.rnn:
                self.best_loss = self.best_loss[0]
#
            self.loss_hist = np.genfromtxt(
                self.model_dir + name_loss_hist(self.epochs_trained), delimiter=',')
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        self.loss_hist = np.concatenate(
            (self.loss_hist, np.array(model_loss).reshape([1, -1])), axis=0)
        if model_loss_main < self.best_loss:
            if self.verbose:
                print('loss improve from {:e} to {:e}'.format(
        ...
        if epochs_trained_updated % self.save_freq == 0:
            self.best_model.save_weights(
                self.model_dir + name_best_model_epoch(epochs_trained_updated))
            self.model.save_weights(
                self.model_dir + name_model_epoch(epochs_trained_updated))
            np.savetxt(self.model_dir + name_loss_hist(epochs_trained_updated),
                       self.loss_hist, delimiter=',')
            for best_sub_model in self.best_sub_models:
                best_sub_model.save_weights(
                    self.model_dir + best_sub_model.name+'_best_{}epochs'.format(epochs_trained_updated))
