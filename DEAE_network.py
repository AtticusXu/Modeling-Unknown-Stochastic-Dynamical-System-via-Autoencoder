...
def make_encoder_model(n_x, n_sample, encoder_dim, latent_dim):
    ...
    encoder_1 = layers.Conv1D(filters=encoder_dim, kernel_size=1, activation="elu")(
        encoder_0
    )
    encoder_2 = layers.Conv1D(filters=encoder_dim, kernel_size=1, activation="elu")(
        encoder_1
    )
    encoder_3 = layers.Conv1D(filters=encoder_dim, kernel_size=1, activation="elu")(
        encoder_2
    )
...
def make_decoder_model(n_x, n_sample, decoder_dim, latent_dim):
    ...
    decoder_1 = layers.Conv1D(filters=decoder_dim, kernel_size=1, activation="elu")(
        decoder_0
    )
    decoder_2 = layers.Conv1D(filters=decoder_dim, kernel_size=1, activation="elu")(
        decoder_1
    )
    decoder_3 = layers.Conv1D(filters=decoder_dim, kernel_size=1, activation="elu")(
        decoder_2
    )
...
class KDELayer1D(layers.Layer):
    def __init__(self, lower=-3.0, upper=3.0, num=101, n_sample=1000):
    ...
    def build(self, input_shape):
        ...
        self.q_broadcast = tf.expand_dims(
            tf.broadcast_to(self.quantiles, q_shape), axis=-1
        )

    def call(self, inputs):
        weights = tf.exp(
            -0.5
            * tf.square((tf.expand_dims(inputs, axis=2) - self.q_broadcast) / self.bw)
        )

        return tf.reduce_sum(weights, axis=1) / self.norm_factor


class KDELayer(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.h = (4 / (self.d + 2)) ** (1 / (self.d + 4)) * self.n_sample ** (
            -1 / (self.d + 4)
        )

    def gaussian_kernel(self, diff):
        return (1 / ((2 * np.pi * self.h**2) ** (self.d / 2))) * tf.exp(
            -0.5 * tf.norm(diff, axis=-1) ** 2 / self.h**2
        )
...
class MMDLayer(layers.Layer):
    ...
    def RBFkernel(self, x, y):
        ...
        return tf.exp(
            -tf.reduce_sum((tiled_x - tiled_y) ** 2, axis=3) / (2 * self.sigma**2)
        )
    ...
    def call(self, x):
        ...
        return (
            x_kernel
            + tf.reduce_sum(y_kernel, axis=[1, 2]) / (n_sample * (n_sample - 1))
            - 2
            / n_sample
            * tf.reduce_sum(xy_kernel, axis=1)
            * (self.sigma**2 / (1 + self.sigma**2)) ** (dim / 2.0)
        )


class GroupMMDLayer(layers.Layer):
    ...
    def call(self, x):
        ...
        for i in range(self.groups):
            ...
            xy_kernel = tf.exp(
                -tf.reduce_sum(x_subgroup**2, axis=2) / (2 * (1 + self.sigma**2))
            )
            loss_mmd_subgroup = (
                x_kernel
                + tf.reduce_sum(y_kernel, axis=[1, 2]) / (gs * (gs - 1))
                - 2
                / gs
                * tf.reduce_sum(xy_kernel, axis=1)
                * (self.sigma**2 / (1 + self.sigma**2)) ** (dim / 2.0)
...
class FastMMDLayer(layers.Layer):
    """
...
class CovarianceLayer1D(layers.Layer):
    ...
    def call(self, inputs):
        ...
        input_A_mean_subtracted = input_A - tf.reduce_mean(
            input_A, axis=1, keepdims=True
        )
        input_B_mean_subtracted = input_B - tf.reduce_mean(
            input_B, axis=1, keepdims=True
        )
        ...
        covariance = tf.matmul(
            input_A_mean_subtracted, input_B_mean_subtracted, transpose_a=True
        )
        covariance = covariance / tf.cast(
            tf.shape(input_A_mean_subtracted)[1] - 1, tf.float32
        )
        covariance = covariance / (sig_A * sig_B)
        return covariance


class CovZLayer(layers.Layer):
    def __init__(
        self,
    ):
    ...
    def call(self, inputs):
        ...
        for i in range(ndim - 1):
            for j in range(i + 1, ndim):
                covZ.append(
                    self.cov_layer([inputs[:, :, i : i + 1], inputs[:, :, j : j + 1]])
                )
...
class ConditionalVAE(keras.Model):
    ...
    def call(self, inputs):
        ...
        self.add_metric(MSE_loss, name="MSE")

        z_moment = self.moment_layer(z)
        moment_loss = keras.losses.MeanSquaredError()(z_moment, self.moment_normal)
        self.add_metric(moment_loss, name="moment")

        z_pdf_1d = self.kde_layer_1d(z)
        kde_1d_loss = keras.losses.MeanSquaredError()(z_pdf_1d, self.pdf_normal_1d)
        self.add_metric(kde_1d_loss, name="kde_1d")

        vae_loss = (
            MSE_loss
            + self.beta_moment * (moment_loss)
            + self.beta_kde_1d * (kde_1d_loss)
        )

        if self.beta_kde_nd > 0:
            z_pdf_nd = self.kde_layer_nd(z)
            kde_nd_loss = keras.losses.MeanSquaredError()(z_pdf_nd, self.pdf_normal_nd)
            vae_loss = vae_loss + self.beta_kde_nd * (kde_nd_loss)
            self.add_metric(kde_nd_loss, name="kde_nd")

        if self.beta_cov > 0:
            z_cov = self.cov_layer(z)
            cov_loss = tf.reduce_mean(tf.math.square(z_cov))
            vae_loss = vae_loss + self.beta_cov * (cov_loss)
            self.add_metric(cov_loss, name="z_cov")
...
if __name__ == "__main__":
    ...
    kde_layer_1d, pdf_normal_c_1d, kde_layer_nd, pdf_normal_c_nd = (
        DEAE_lib.kde_preparation(
            "Ex14OU5D2", latent_dim, "combine", [-3.0, 3.0], 100, n_sample, batch
        )
    )
    ...
    p_loss_nd = keras.losses.MeanSquaredError()(
        pdf_nd, pdf_normal_c_nd
    ).numpy() * 5.0 ** (latent_dim - 2)
