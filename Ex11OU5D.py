...
#%%
    for i in range(2):
        ...
        for j in range(100):
            plt.scatter(data_sort[j,:,p_ax_x,0], data_sort[j,:,p_ax_y,0], s=0.1)
        plt.xlabel(f'x_{p_ax_x}')
        plt.ylabel(f'x_{p_ax_y}')
        plt.title(eqn_name+' neighbor {} scatter'.format(n_sample[i]))
        plt.savefig('data/'+eqn_name+'/neighbor_space_train_{}_{}v{}.jpg'.format(i+1,p_ax_x,p_ax_y),dpi=600)
        plt.show()
        plt.close('all')

        for t in range(n_sample[i]):
            plt.plot(data_sort[0,t,p_ax_x,:], data_sort[0,t,p_ax_y,:], c = 'yellow', linewidth=0.3)
        plt.scatter(data_sort[0,:,p_ax_x,0], data_sort[0,:,p_ax_y,0], c = 'red', s=0.5,label = 'x(0)')
        plt.scatter(data_sort[0,:,p_ax_x,1], data_sort[0,:,p_ax_y,1], c = 'blue', s=0.5,label = 'x(dt)')
        plt.title(eqn_name+' neighbor {} 1 step trajectories'.format(n_sample[i]))
        plt.xlabel(f'x_{p_ax_x}')
        plt.ylabel(f'x_{p_ax_y}')
        plt.legend()
        plt.savefig('data/'+eqn_name+'/neighbor_train_traj_{}_{}v{}.jpg'.format(i+1,p_ax_x,p_ax_y),dpi=600)
        plt.show()
        plt.close('all')
#%%   
    # generate test data
